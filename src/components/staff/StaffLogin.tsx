/**
 * Formulaire de connexion staff (cuisine, commandes…).
 *
 * Auth Payload : POST /api/users/login puis GET /api/staff/session.
 */
'use client'
...
export function StaffLogin({ site, pageTitle, loginDescription, afterSubtitle, onSuccess }: Props) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const sessionResponse = await fetch(`/api/staff/session?siteId=${site.id}`, {
        credentials: 'include',
        cache: 'no-store',
      })
      ...
      if (!sessionResponse.ok) {
        ...
        throw new Error(
          sessionData.message ?? 'Compte sans accès à ce restaurant.',
        )
      }
    ...
    <div className="mx-auto flex min-h-[60vh] max-w-md flex-col justify-center px-4 py-12">
      <div className="space-y-2 text-center">
        ...
        <p className="text-sm text-muted-foreground">
          {loginDescription ?? `Connectez-vous pour gérer les commandes de ${site.name}.`}
        </p>
      </div>
